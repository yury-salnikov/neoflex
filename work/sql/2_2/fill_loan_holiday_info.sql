...
as $$
begin
	create table if not exists dm2.loan_holiday_info (like dm.loan_holiday_info);
	insert into dm2.loan_holiday_info select * from dm.loan_holiday_info;
	...
	left join product p on p.product_rk = d.product_rk
						   and p.effective_from_date = d.effective_from_date and p.product_name = d.deal_name
	)
	SELECT deal_rk
	      ,effective_from_date
	      ,effective_to_date
